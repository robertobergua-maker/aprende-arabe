...
// --- DATOS COMPLETOS (FORMATO CORREGIDO) ---
const INITIAL_DATA = [
  // Pista 1: Saludos
...
export default function App() {
  ...
  const [cards, setCards] = useState(() => {
    try {
      // ACTUALIZADO A v6 PARA RECARGAR DATOS LIMPIOS
      const saved = localStorage.getItem('flashcards-data-v6');
  ...
  // ESTADO NUEVO: Controla qué idioma se muestra al frente
  const [frontLanguage, setFrontLanguage] = useState("spanish"); // "spanish" | "arabic"
  ...
  useEffect(() => {
    try {
      localStorage.setItem('flashcards-data-v6', JSON.stringify(cards));
    } catch (e) {
      ...
      <header className="bg-emerald-700 text-white shadow-md z-20">
        <div className="w-full px-4 py-3 flex justify-between items-center gap-4">
          ...
          <div className="flex-1 max-w-xl mx-4 flex gap-2">
            ...
            {/* SELECTOR DE IDIOMA */}
            <div className="flex items-center bg-emerald-800/50 rounded-lg p-1 border border-emerald-600/30 flex-shrink-0">
                <button
            ...
            {filteredCards.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
                {filteredCards.map(card => (
                  <Flashcard key={card.id} data={card} frontLanguage={frontLanguage} />
                ))}
              </div>
            )}
          </div>
...
// --- TARJETA ---
function Flashcard({ data, frontLanguage }) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-scale-in flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 flex flex-col">
          ...
          <textarea 
            ...
            placeholder='Ej: [{"spanish": "Hola", "arabic": "مرحبا"}]'
            value={jsonText}
